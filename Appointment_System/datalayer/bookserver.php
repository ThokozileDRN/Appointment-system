...
if(count($errors)==0){
	...
	$sql = "INSERT INTO  bookapp (AppoID, Date, Time, UserID, agentID) VALUES ('$AppoID','$Date','$Time','$userprofile','$agentID') ";
...
if (isset($_POST['cancel'])) {
 ...
 if (count($errors)==0) {
	...
	$query5="DELETE FROM bookapp WHERE AppoID='$AppoID2' AND userID=('$userprofile') ";
...
if(count($errors)==0){
	...
	$sqladd = "INSERT INTO  agent (AgentID, Agentname, email, Address, ContactNumber, password,categorey) VALUES ('$addID','$addname','$addEmail','$addAddress','$addContactNumber','$addPassword','$addcategory') ";
...
if (isset($_POST['Delete'])) {
 ...
 if (count($errors)==0) {
	...
	$querydelete="DELETE FROM agent WHERE AgentID='$deleteID' ";
	if ($mysqli -> query($querydelete)) {

		if ($mysqli->affected_rows==0) {
			 array_push($errors,"Wrong Agent ID");
...
<!-- $query1="SELECT categorey FROM Agent";
